...
struct DB_env {
	...
	rocksdb_filterpolicy_t *filterpolicy;
	rocksdb_t *db;
	MDB_env *tmpenv;
...
static int ldb_cursor_open(rocksdb_t *const db, rocksdb_readoptions_t *const ropts, MDB_cmp_func *const cmp, LDB_cursor **const out) {
	...
	cursor->iter = rocksdb_create_iterator(db, ropts);
	cursor->cmp = cmp;
	if(!cursor->iter) {
...
static void ldb_cursor_close(LDB_cursor *const cursor) {
	...
	rocksdb_iter_destroy(cursor->iter);
	cursor->cmp = NULL;
	cursor->valid = 0;
	free(cursor);
}
...
static int ldb_cursor_current(LDB_cursor *const cursor, MDB_val *const key, MDB_val *const val) {
	...
	if(key) {
		...
		size_t s;
		char const *const x = (char *)rocksdb_iter_key(cursor->iter, &s);
	...
	if(val) {
		...
		size_t s;
		char const *const x = (char *)rocksdb_iter_value(cursor->iter, &s);
		char *const y = malloc(s);
		if(!y) return DB_ENOMEM;
		memcpy(y, x, s);
...
static int ldb_cursor_seek(LDB_cursor *const cursor, MDB_val *const key, MDB_val *const val, int const dir) {
	...
	MDB_val const orig = *key;
	rocksdb_iter_seek(cursor->iter, key->mv_data, key->mv_size);
	cursor->valid = !!rocksdb_iter_valid(cursor->iter);
	int rc = ldb_cursor_current(cursor, key, val);
	if(dir > 0) return rc;
	if(dir < 0) {
		if(DB_SUCCESS != rc) {
			rocksdb_iter_seek_to_last(cursor->iter);
		} else if(0 != cursor->cmp(key, &orig)) {
			rocksdb_iter_prev(cursor->iter);
		} else return rc;
		cursor->valid = !!rocksdb_iter_valid(cursor->iter);
...
static int ldb_cursor_first(LDB_cursor *const cursor, MDB_val *const key, MDB_val *const val, int const dir) {
	...
	if(dir > 0) rocksdb_iter_seek_to_first(cursor->iter);
	if(dir < 0) rocksdb_iter_seek_to_last(cursor->iter);
	cursor->valid = !!rocksdb_iter_valid(cursor->iter);
	return ldb_cursor_current(cursor, key, val);
}
static int ldb_cursor_next(LDB_cursor *const cursor, MDB_val *const key, MDB_val *const val, int const dir) {
	...
	if(0 == dir) return DB_EINVAL;
	if(dir > 0) rocksdb_iter_next(cursor->iter);
	if(dir < 0) rocksdb_iter_prev(cursor->iter);
	cursor->valid = !!rocksdb_iter_valid(cursor->iter);
	return ldb_cursor_current(cursor, key, val);
}
...
int db_env_create(DB_env **const out) {
	...
	env->opts = rocksdb_options_create();
	if(!env->opts) {
		db_env_close(env);
		return DB_ENOMEM;
	}
...
//#endif
	rocksdb_options_set_max_open_files(env->opts, maxfiles);
...
/*	env->filterpolicy = rocksdb_filterpolicy_create_bloom(10);
	...
	env->wopts = rocksdb_writeoptions_create();
	if(!env->wopts) {
		db_env_close(env);
		return DB_ENOMEM;
	}
	rocksdb_writeoptions_set_sync(env->wopts, 1);
...
int db_env_open(DB_env *const env, char const *const name, unsigned const flags, unsigned const mode) {
	...
	char *err = NULL;
	env->db = rocksdb_open(env->opts, name, &err);
	free(err);
	...
	rocksdb_writeoptions_set_sync(env->wopts, !(DB_NOSYNC & flags));
	return DB_SUCCESS;
}
void db_env_close(DB_env *const env) {
	if(!env) return;
	rocksdb_options_destroy(env->opts);
	rocksdb_close(env->db);
	rocksdb_writeoptions_destroy(env->wopts);
...
int db_txn_begin(DB_env *const env, DB_txn *const parent, unsigned const flags, DB_txn **const out) {
	...
	txn->parent = parent;
	txn->flags = flags;
	txn->ropts = rocksdb_readoptions_create();
	txn->tmptxn = tmptxn;
	if(!txn->ropts) {
		db_txn_abort(txn);
		return DB_ENOMEM;
...
int db_txn_commit(DB_txn *const txn) {
	...
	char *err = NULL;
	rocksdb_write(txn->env->db, txn->env->wopts, batch, &err);
	free(err);
...
void db_txn_abort(DB_txn *const txn) {
	...
	rocksdb_readoptions_destroy(txn->ropts);
...
void db_txn_reset(DB_txn *const txn) {
	...
	rocksdb_readoptions_set_snapshot(txn->ropts, NULL);
}
int db_txn_renew(DB_txn *const txn) {
	// TODO: If renew fails, does the user have to explicitly abort?
	if(!txn) return DB_EINVAL;
	assert(txn->flags & DB_RDONLY);
	rocksdb_snapshot_t const *snapshot = rocksdb_create_snapshot(txn->env->db);
	if(!snapshot) return DB_ENOMEM;
	rocksdb_readoptions_set_snapshot(txn->ropts, snapshot);
	return DB_SUCCESS;
}
