...
err_t EFSMetaFileStore(EFSMetaFileRef const meta, int64_t const fileID, sqlite3 *const db);
...
err_t EFSMetaFileWrite(EFSMetaFileRef const meta, byte_t const *const buf, size_t const len) {
	...
	return yajl_status_ok == yajl_parse(meta->parser, buf, len) ? 0 : -1;
}
err_t EFSMetaFileEnd(EFSMetaFileRef const meta) {
	if(!meta) return 0;
	if(meta->size > META_MAX) return -1;
	return yajl_status_ok == yajl_complete_parse(meta->parser) ? 0 : -1;
}
...
static int yajl_boolean(EFSMetaFileRef const meta, int const flag) {
	switch(meta->state) {
		case s_skip: break;
		case s_visible: meta->visible = flag; break;
		default: return -1;
	}
	return 0;
}
...
static int yajl_string(EFSMetaFileRef const meta, strarg_t const str, size_t const len) {
	switch(meta->state) {
		case s_skip: break;
		case s_meta_uri: meta->URI = strndup(str, len); break;
		case s_link: URIListAddURI(meta->links, str, len); break;
		case s_title: meta->title = strndup(str, len); break;
		case s_desc: meta->desc = strndup(str, len); break;
		default: return -1;
	}
	return 0;
}
...
static int yajl_map_key(EFSMetaFileRef const meta, strarg_t const key, size_t const len) {
	switch(meta->state) {
		...
		case s_top:
			...
			break;
		default: BTAssert(0, "Unexpected map key");
	}
	return 0;
}
static int yajl_end_map(EFSMetaFileRef const meta) {
	switch(meta->state) {
		...
		default: BTAssert(0, "Unexpected map end");
	}
	return 0;
}
...
static int yajl_end_array(EFSMetaFileRef const meta) {
	switch(meta->state) {
		...
		default: BTAssert(0, "Unexpected array end");
	}
	return 0;
}
static yajl_callbacks const callbacks = {
