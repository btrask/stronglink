...
dupsort is cool but it isnt actually key-value
...
int lsmdb_env_wrap(MDB_env *const env, LSMDB_env **const out);
void lsmdb_env_close(LSMDB_env *const env);

int lsmdb_dbi_open(LSMDB_env *const env, char const *const name, unsigned const flags, LSMDB_dbi *const out);
...
int lsmdb_get(LSMDB_txn *const txn, MDB_dbi const dbi, MDB_val *const key, MDB_val *const data);
int lsmdb_put(LSMDB_txn *const txn, MDB_dbi const dbi, MDB_val *const key, MDB_val *const data, unsigned const flags);
int lsmdb_del(LSMDB_txn *const txn, MDB_dbi const dbi, MDB_val *const key);

int lsmdb_cursor_open(LSMDB_txn *const txn, MDB_dbi const dbi, LSMDB_cursor **const cursor);
