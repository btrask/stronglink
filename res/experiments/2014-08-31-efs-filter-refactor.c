...
- (size_t)getUserFilter:(str_t *const)buf :(size_t const)size :(count_t const)depth;
...
- (EFSFilter *)unwrap;
- (void)addFilterArg(EFSFilter *const)filter;
- (uint64_t)step:(int const)dir;
...
- (uint64_t)age:(uint64_t const)fileID :(uint64_t const)sortID {
	...
	for(; MDB_SUCCESS == rc; rc = mdb_cursor_get(age_uris, fileID_val, URI_val, MDB_NEXT_DUP)) {
		...
		for(; MDB_SUCCESS == rc; rc = mdb_cursor_get(age_metafiles, targetURI_val, metaFileID_val, MDB_NEXT_DUP)) {
			uint64_t const age = db_column(metaFileID_val, 0);
			if(age > sortID) break;
...
static int filtercmp(EFSFilter *const _a, EFSFilter *const _b) {
...
- (err_t)addFilterArg(EFSFilter *const)filter {
	...
	if(count+1 > size) {
		size = MAX(8, size * 2);
		filters = realloc(filters, sizeof(filters[0]) * size);
		assert(filters); // TODO
	}
...
- (void)sort {
	qsort(filters, count, sizeof(filters[0]), filtercmp);
}
@end
...
- (err_t)addStringArg:(strarg_t const)str :(size_t const)len {
	...
	for(;;) {
		...
		if(count+1 > size) {
			size = MAX(8, size*2);
			tokens = realloc(tokens, sizeof(tokens[0]) * size);
			assert(tokens); // TODO
		}
...
- (size_t)getUserFilter:(str_t *const)buf :(size_t const)size :(count_t const)depth {
	return wr_quoted(buf, size, term);
}

- (err_t)prepare:(MDB_txn *const)txn :(EFSConnection const *const)conn {
	if([super prepare:txn :conn] < 0) return -1;
	db_cursor(txn, conn->metaFileIDByFulltext, step_metafiles);
	db_cursor(txn, conn->fileIDByURI, step_uris);
