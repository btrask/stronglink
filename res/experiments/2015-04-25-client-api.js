...
stuff clients want to do
- perform a query and get the uris/files back
- watch a query for updates
- load files
- get file meta-data
- submit files/meta-files
- connect to either the default repo or a specific one

for clients, a "repo" object has an implicit session
otherwise you wouldnt be able to do anything...

we should store cookies, not passwords, for automatic connection
so we probably dont need to support password based connection at all...?
have a separate tool for logging in and fetching the cookie
pulls should probably work the same way...


...we're really using streams like crazy
although theyre they only way to do things sanely in node, i guess?

...are these the only operations we need?
...
(function() {
//var sln = require("/home/user/Desktop/2015-04-25-client-api");
//var repo = new sln.Repo("http://localhost:8000", "1:69a39f07f2dbefdb0ffbaf66e9d36d4f")
//repo.createQueryStream("", {}).pipe(process.stdout);

//console.log(sln.parseURI("hash://asdf/asdf"));
})();
